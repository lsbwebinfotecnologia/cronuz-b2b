...
export default function HorusConsignmentManager({ apiBaseUrl, token, backUrl }: HorusConsignmentManagerProps) {
    ...
    const [pendingSubmitConfirm, setPendingSubmitConfirm] = useState(false);
    ...
    // Prevent auto-save while fetching
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            let activeCodCtr = summaryData.COD_CTR || (Array.isArray(summaryData) ? summaryData[0]?.COD_CTR : null);

            // Fetch Draft before normalizing items so we can merge
            let draftItems: any[] = [];
            let draftOpType: "A" | "D" = "A";
            if (activeCodCtr) {
                try {
                    const draftRes = await fetch(`${baseUrl}${apiBaseUrl}/draft?cod_ctr=${activeCodCtr}`, {
                        headers: { 'Authorization': `Bearer ${token}` }
                    });
                    if (draftRes.ok) {
                        const draftData = await draftRes.json();
                        draftItems = draftData.items_json || [];
                        if (draftData.operation_type === "A" || draftData.operation_type === "D") {
                            draftOpType = draftData.operation_type;
                        }
                    }
                } catch (e) {
                    console.log("No draft found or error fetching draft", e);
                }
            }

            const detailedArray = Array.isArray(detailsData) ? detailsData : [detailsData];
            
            const divs: {barcode: string, oldDraftQty: number, newHorusMax: number}[] = [];

            const normalizedItems = detailedArray.map(item => {
                const barcode = item.COD_BARRA_ITEM;
                const draftMatch = draftItems.find(d => d.COD_BARRA_ITEM === barcode);
                let loadedQty = draftMatch ? (draftMatch.qtdConferida || 0) : 0;
                
                const currentHorusBalance = item.SALDO_ITENS || 0;
                
                if (loadedQty > currentHorusBalance) {
                    divs.push({ barcode, oldDraftQty: loadedQty, newHorusMax: currentHorusBalance });
                    loadedQty = currentHorusBalance; // Cap it safely to the new Horus reality
                }
                ...
                return {
                    ...
                    SALDO_ITENS: currentHorusBalance,
                    VLR_TOTAL_LIQUIDO: item.VLR_TOTAL_LIQUIDO || 0,
                    qtdConferida: loadedQty
                };
            });
            
            fetchingRef.current = true;
            setItems(normalizedItems);
            setOperationType(draftOpType);

            if (divs.length > 0) {
                setDivergenceItems(divs);
                setShowDivergenceModal(true);
            }
            
            // Allow autosaving again after a tiny delay
    ...
    // Auto-Save Draft
    useEffect(() => {
        if (fetchingRef.current || !summary?.COD_CTR) return;
        const itemsToSave = items.filter(x => x.qtdConferida > 0);
        
        // Always save even if empty (to allow clearing the draft)
        const saveDraft = async () => {
            try {
                const baseUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
                await fetch(`${baseUrl}${apiBaseUrl}/draft`, {
                    method: 'POST',
                    headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        cod_ctr: String(summary.COD_CTR),
                        operation_type: operationType,
                        items_json: itemsToSave.map(i => ({
                            COD_BARRA_ITEM: i.COD_BARRA_ITEM,
    ...
    }, [items, operationType, summary?.COD_CTR, apiBaseUrl, token]);
    ...
    const handleDownloadTemplate = () => {
        const worksheet = XLSX.utils.json_to_sheet([{ isbn: "", qtd: "" }]);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "Modelo");
        XLSX.writeFile(workbook, "modelo_consignacao.xlsx");
    };
    ...
    const submitBatch = async () => {
         ...
         // Em vez de submit direto, abre a modal visual de Confirmação e Resumo
         setPendingSubmitConfirm(true);
    };

    const confirmAndExecuteSubmit = async () => {
         setPendingSubmitConfirm(false);
         const itemsToSubmit = items.filter(x => x.qtdConferida > 0);
         const payload = {
        ...
        <div className="flex flex-col h-full bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm relative">
             ...
             <div className="bg-slate-50 border-b border-slate-200 p-6 flex flex-col md:flex-row md:items-start justify-between gap-6">
                  ...
                  {summary && (
                      <div className="flex flex-wrap items-center gap-4 bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
                           ...
                           {/* LIVE DRAFT SUMMARY */}
                           <div className="w-px h-8 bg-slate-200 hidden sm:block"></div>
                           <div className="bg-indigo-50 px-3 py-1.5 rounded-lg border border-indigo-100 flex flex-col justify-center">
             ...
             <div className="p-4 border-b border-slate-100 flex flex-col xl:flex-row gap-4 items-end bg-white">
                  ...
                  <div className="flex-none min-w-[200px] w-full xl:w-auto">
                       ...
                       <div className="flex p-1 bg-slate-100 rounded-lg border border-slate-200">
                            <button 
                                ...
                                className={`flex-1 py-1 text-xs font-bold rounded-md transition-all ${operationType === 'A' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-800'}`}
                            >
                                Acerto
                            </button>
                            <button 
                                ...
                                className={`flex-1 py-1 text-xs font-bold rounded-md transition-all ${operationType === 'D' ? 'bg-rose-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-800'}`}
                            >
                                Devolução
                            </button>
                       </div>
                  ...
                  <div className="flex-none min-w-[200px] w-full xl:w-auto">
                       ...
                       <div className="flex p-1 bg-slate-100 rounded-lg border border-slate-200">
                            <button 
                                ...
                                className={`flex-1 py-1 text-xs font-bold flex items-center justify-center gap-1 rounded-md transition-all ${scanMode === 'geral' ? 'bg-white text-slate-800 shadow-sm border border-slate-200/60' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                <Calculator className="w-3.5 h-3.5" /> Digitar Qtd.
                            </button>
                            <button 
                                onClick={() => {
                                ...
                                className={`flex-1 py-1 text-xs font-bold flex items-center justify-center gap-1 rounded-md transition-all ${scanMode === 'contador' ? 'bg-white text-slate-800 shadow-sm border border-slate-200/60' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                <ScanLine className="w-3.5 h-3.5" /> Bipar (+1)
                            </button>
                       </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 w-full xl:w-auto">
                       ...
                       <button onClick={handleDownloadTemplate} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 border border-slate-200 rounded-lg flex items-center gap-2 transition-colors cursor-pointer text-sm font-bold whitespace-nowrap" title="Baixar Modelo de Planilha">
             ...
             {showDivergenceModal && (
                  <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in transition-all">
                      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full text-left overflow-hidden ring-4 ring-[#FFD700]/30 animate-in zoom-in-95">
                           <div className="bg-[#FFD700] p-6 flex items-start gap-4">
                                <AlertTriangle className="w-8 h-8 text-yellow-900 mt-1 flex-shrink-0" />
                                <div>
                                    <h3 className="text-xl font-black text-yellow-900">Atenção! Divergência Detectada com Horus</h3>
                                    <p className="text-yellow-800 text-sm font-medium mt-1">
                                        Seu rascunho salvo possuía quantidades maiores do que o saldo atual reportado no ERP Horus agorinha. As quantidades em tela foram automaticamente abaixadas para o teto possível por segurança.
                                    </p>
                                </div>
                           </div>
                           <div className="p-6 max-h-[50vh] overflow-y-auto">
                                <table className="w-full text-sm text-left border border-slate-200">
                                    <thead className="bg-slate-50 text-slate-500 font-bold">
                                        <tr>
                                            <th className="px-4 py-2 border-b">Cód. Barras</th>
                                            <th className="px-4 py-2 border-b text-center">Quant. Rascunho</th>
                                            <th className="px-4 py-2 border-b text-center text-rose-600">Novo Teto Horus</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-100 font-medium">
                                        {divergenceItems.map((div, i) => (
                                            <tr key={i}>
                                                <td className="px-4 py-3 font-mono">{div.barcode}</td>
                                                <td className="px-4 py-3 text-center">{div.oldDraftQty}</td>
                                                <td className="px-4 py-3 text-center font-bold text-rose-600">{div.newHorusMax}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                           ...
                           <div className="bg-slate-50 p-4 border-t border-slate-200 flex justify-end">
                               <button 
                                   ...
                                   className="px-6 py-2.5 bg-yellow-600 hover:bg-yellow-700 text-white font-bold rounded-lg transition-colors"
                               >
                                   Entendi, Prosseguir
                               </button>
                           </div>
             ...
             {/* PRE-SUBMIT CONFIRMATION SUMMARY MODAL */}
             {pendingSubmitConfirm && (() => {
                  const itemsToSubmit = items.filter(x => x.qtdConferida > 0);
                  return (
                      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                           <div className="bg-white rounded-2xl w-full max-w-4xl shadow-2xl relative animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
                                <div className="p-6 border-b border-slate-200 shrink-0">
                                     <button onClick={() => setPendingSubmitConfirm(false)} className="absolute top-6 right-6 text-slate-400 hover:text-slate-600">
                                          <XCircle className="w-6 h-6" />
                                     </button>
                                     <div className="flex items-center gap-3">
                                          <BookOpen className="w-8 h-8 text-[var(--color-primary-base, #3b82f6)]" />
                                          <div>
                                              <h3 className="text-xl font-bold text-slate-800">Resumo da Operação</h3>
                                              <p className="text-sm text-slate-500">Revise os itens antes de disparar o(a) {operationType === 'A' ? 'ACERTO' : 'DEVOLUÇÃO'} para o Horus ERP.</p>
                                          </div>
                                     </div>
                                </div>
                                ...
                                <div className="overflow-y-auto p-0 flex-1 bg-slate-50">
                                    <table className="w-full text-left text-sm whitespace-nowrap text-slate-600">
                                      <thead className="bg-white sticky top-0 border-b border-slate-200 box-border text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                                          <tr>
                                              <th className="px-6 py-3">Cód. Barras (ISBN)</th>
                                              <th className="px-6 py-3">Título do Item</th>
                                              <th className="px-6 py-3 text-right">Preço Un.</th>
                                              <th className={`px-6 py-3 text-center ${operationType === 'A' ? 'text-indigo-600 bg-indigo-50/50' : 'text-rose-600 bg-rose-50/50'}`}>QTD</th>
                                              <th className="px-6 py-3 text-right text-slate-800">Subtotal</th>
                                          </tr>
                                      </thead>
                                      <tbody className="divide-y divide-slate-100 bg-white">
                                          {itemsToSubmit.map((item, i) => {
                                              const price = parseFloat(item.VLR_LIQUIDO.replace(',', '.')) || 0;
                                              const subtotal = price * item.qtdConferida;
                                              return (
                                                  <tr key={i} className="hover:bg-slate-50 transition-colors">
                                                      <td className="px-6 py-3 font-mono font-medium">{item.COD_BARRA_ITEM}</td>
                                                      <td className="px-6 py-3 truncate max-w-[200px]" title={item.NOM_ITEM}>{item.NOM_ITEM}</td>
                                                      <td className="px-6 py-3 text-right">R$ {item.VLR_LIQUIDO}</td>
                                                      <td className="px-6 py-3 text-center font-bold text-slate-800">{item.qtdConferida}</td>
                                                      <td className="px-6 py-3 text-right font-medium">R$ {subtotal.toFixed(2).replace('.', ',')}</td>
                                                  </tr>
                                              )
                                          })}
                                      </tbody>
                                ...
                                <div className="p-6 border-t border-slate-200 shrink-0 bg-slate-50 rounded-b-2xl flex flex-col md:flex-row items-center justify-between gap-6">
                                     <div className="flex gap-8">
                                         <div>
                                             <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Itens Únicos</p>
                                             <p className="text-xl font-bold text-slate-800">{itemsToSubmit.length}</p>
                                         </div>
                                         <div>
                                             <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Qtd Acumulada</p>
                                             <p className="text-xl font-bold text-slate-800">{totalQtdApurada}</p>
                                         </div>
                                         <div>
                                             <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Total a Enviar</p>
                                             <p className="text-2xl font-black text-slate-900">R$ {totalValorApurado.toFixed(2).replace('.', ',')}</p>
                                         </div>
                                     </div>
                                     <button 
                                         onClick={confirmAndExecuteSubmit} 
                                         className={`px-8 py-3 rounded-xl font-bold text-white shadow-sm transition-all focus:ring-4 focus:outline-none ${operationType === 'A' ? 'bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500/20 shadow-indigo-600/20' : 'bg-rose-600 hover:bg-rose-700 focus:ring-rose-500/20 shadow-rose-600/20'}`}
                                     >
                                         Confirmar e Enviar Acerto Definitivo
                                     </button>
                                </div>
                           </div>
                      </div>
