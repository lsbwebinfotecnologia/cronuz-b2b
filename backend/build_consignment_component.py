...
import { Loader2, Search, CheckCircle2, AlertTriangle, Upload, RefreshCw, XCircle, FileSpreadsheet, ArrowRight, BookOpen } from 'lucide-react';
...
export default function HorusConsignmentManager({ apiBaseUrl, token, backUrl }: HorusConsignmentManagerProps) {
    ...
    const handleBarcodeSubmit = (e: React.FormEvent) => {
        ...
        // Validação de saldo ao adicionar +1
        ...
        // Scroll to item element intentionally simple, just let user know
    };
    ...
    const submitBatch = async () => {
         ...
         try {
             ...
             toast.success(`Operação de ${operationType === 'A' ? 'ACERTO' : 'DEVOLUÇÃO'} finalizada com sucesso! Verifique a resposta visual para mais detalhes.`);
             // Recarregar os saldos recém-frescos
             await fetchData();
         } catch(err: any) {
    ...
    return (
        <div className="flex flex-col h-full bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
             ...
             {/* Command Center */}
             <div className="p-4 border-b border-slate-100 flex flex-col lg:flex-row gap-4 items-end bg-white">
                       ...
                       <div className="flex p-1 bg-slate-100 rounded-lg border border-slate-200">
                            <button 
                                onClick={() => React.startTransition(() => {
                                    // if changing operations, clear conferidas. Safety guarantee.
                  ...
                  <div className="flex items-center gap-2 lg:ml-auto w-full lg:w-auto">
                       ...
                       <button onClick={fetchData} className="p-2 border border-slate-200 bg-white hover:bg-slate-50 text-slate-500 rounded-lg transition-colors cursor-pointer">
             ...
             <div className="bg-slate-50 border-t border-slate-200 p-4 shrink-0 flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-6 divide-x divide-slate-200">
                       <div className="pr-6">
                            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-0.5">Total Itens Conferidos</p>
                  ...
                  >
                      {submitting ? (
                          ...
                          <><CheckCircle2 className="w-5 h-5"/> Gravar {operationType === 'A' ? 'Acerto' : 'Devolução'}</>
                      )}
                  </button>
