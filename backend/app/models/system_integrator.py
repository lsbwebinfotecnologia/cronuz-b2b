...
class SystemIntegratorGroup(Base):
    __tablename__ = "sys_integrator_group"
    
    id = Column(Integer, primary_key=True, index=True)
    system_integrator_id = Column(Integer, ForeignKey("sys_integrator.id"), nullable=False)
    name = Column(String(100), nullable=False)
...
class SystemIntegratorField(Base):
    __tablename__ = "sys_integrator_field"
    
    id = Column(Integer, primary_key=True, index=True)
    group_id = Column(Integer, ForeignKey("sys_integrator_group.id"), nullable=False)
